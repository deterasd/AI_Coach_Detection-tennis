import json
import numpy as np
from scipy.signal import savgol_filter
import time
import math

def calculate_angle(p1, p2, p3):
    # Calculate angle formed by three points, returns angle in degrees (0-180)
    def distance(a, b):
        return math.sqrt((a["x"] - b["x"])**2 + (a["y"] - b["y"])**2)

    a = distance(p2, p3)
    b = distance(p1, p3)
    c = distance(p1, p2)

    if a * b == 0:
        return 0

    cos_value = (a**2 + b**2 - c**2) / (2 * a * b)
    cos_value = max(-1, min(1, cos_value))  # Clamp to [-1, 1] for numerical stability

    return math.degrees(math.acos(cos_value))

def vectorized_interpolate(coords_list):
    # Vectorized interpolation for missing coordinates.
    results = []
    for x_coords, y_coords in coords_list:
        # Convert to numpy arrays, replacing None with NaN
        x_arr = np.array([x if x is not None else np.nan for x in x_coords], dtype=float)
        y_arr = np.array([y if y is not None else np.nan for y in y_coords], dtype=float)

        valid_mask = ~(np.isnan(x_arr) | np.isnan(y_arr))

        if np.sum(valid_mask) > 1:
            indices = np.arange(len(x_arr))
            invalid_mask = ~valid_mask

            if np.any(invalid_mask):
                # Linear interpolation for missing values
                x_arr[invalid_mask] = np.interp(indices[invalid_mask], indices[valid_mask], x_arr[valid_mask])
                y_arr[invalid_mask] = np.interp(indices[invalid_mask], indices[valid_mask], y_arr[valid_mask])

        results.append((x_arr, y_arr))
    return results

def smooth_2D_trajectory(input_file, window_length=15, polyorder=3, tennis_window_length=7, tennis_polyorder=2):
    # Smooth 2D trajectory data using Savitzky-Golay filter.
    with open(input_file, 'r') as f:
        data = json.load(f)


    for frame in data:
        frame['tennis_ball_hit'] = False

    # Body keypoint names
    keypoints = [
        "nose", "left_eye", "right_eye", "left_ear", "right_ear",
        "left_shoulder", "right_shoulder", "left_elbow", "right_elbow",
        "left_wrist", "right_wrist", "left_hip", "right_hip",
        "left_knee", "right_knee", "left_ankle", "right_ankle"
    ]

    # === Tennis ball smoothing ===
    # Find valid range (first and last frame with tennis ball detected)
    first_valid = None
    last_valid = None
    for i, frame in enumerate(data):
        if frame['tennis_ball']['x'] is not None and frame['tennis_ball']['y'] is not None:
            if first_valid is None:
                first_valid = i
            last_valid = i

    if first_valid is not None and last_valid is not None:
        valid_range_data = data[first_valid:last_valid + 1]
        frames = np.array([frame['frame'] for frame in valid_range_data])

        tb_x = np.array([frame['tennis_ball']['x'] for frame in valid_range_data], dtype=float)
        tb_y = np.array([frame['tennis_ball']['y'] for frame in valid_range_data], dtype=float)

        # Interpolate missing tennis ball positions
        valid_tb = ~np.isnan(tb_x)
        if np.any(~valid_tb):
            tb_x[~valid_tb] = np.interp(frames[~valid_tb], frames[valid_tb], tb_x[valid_tb])
            tb_y[~valid_tb] = np.interp(frames[~valid_tb], frames[valid_tb], tb_y[valid_tb])

        # Apply Savitzky-Golay filter if enough data points
        if len(tb_x) > tennis_window_length:
            tb_x_smooth = savgol_filter(tb_x, tennis_window_length, tennis_polyorder)
            tb_y_smooth = savgol_filter(tb_y, tennis_window_length, tennis_polyorder)
        else:
            tb_x_smooth = tb_x
            tb_y_smooth = tb_y

        # Calculate tennis ball trajectory angles
        for i in range(len(valid_range_data)):
            if i > 0 and i < len(valid_range_data) - 1:
                p1 = {"x": tb_x_smooth[i-1], "y": tb_y_smooth[i-1]}
                p2 = {"x": tb_x_smooth[i], "y": tb_y_smooth[i]}
                p3 = {"x": tb_x_smooth[i+1], "y": tb_y_smooth[i+1]}
                angle = calculate_angle(p1, p2, p3)
            else:
                angle = 0

            data[i + first_valid]['tennis_ball'].update({
                'x': float(tb_x_smooth[i]),
                'y': float(tb_y_smooth[i])
            })
            data[i + first_valid]['tennis_ball_angle'] = float(angle)

        # Detect hit point (frame with minimum x coordinate)
        if len(tb_x_smooth) > 0:
            min_x_idx = np.argmin(tb_x_smooth)
            hit_frame_idx = min_x_idx + first_valid
            data[hit_frame_idx]['tennis_ball_hit'] = True
            print(f"Hit point detected at frame {data[hit_frame_idx]['frame']}")

    # Set tennis_ball_angle to 0 for frames outside valid range
    for i in range(len(data)):
        if first_valid is None or i < first_valid or i > last_valid:
            data[i]['tennis_ball_angle'] = 0

    # === Body keypoints batch smoothing ===
    # Collect all keypoint coordinates
    keypoint_coords = []
    for keypoint in keypoints:
        x_coords = [frame[keypoint]['x'] for frame in data]
        y_coords = [frame[keypoint]['y'] for frame in data]
        keypoint_coords.append((x_coords, y_coords))

    # Vectorized interpolation
    interpolated_keypoints = vectorized_interpolate(keypoint_coords)

    # Batch smoothing
    for idx, keypoint in enumerate(keypoints):
        x_array, y_array = interpolated_keypoints[idx]

        # Check if enough valid points for smoothing
        original_x = [frame[keypoint]['x'] for frame in data]
        valid_count = sum(1 for x in original_x if x is not None)

        if valid_count > window_length:
            smooth_x = savgol_filter(x_array, window_length, polyorder)
            smooth_y = savgol_filter(y_array, window_length, polyorder)

            # Update only originally valid points
            for i in range(len(data)):
                if original_x[i] is not None:
                    data[i][keypoint].update({
                        'x': float(smooth_x[i]),
                        'y': float(smooth_y[i])
                    })

    # === Paddle keypoints batch smoothing ===
    paddle_points = ["top", "right", "bottom", "left", "grip_top", "grip_bottom", "center"]
    paddle_coords = []

    # Collect all paddle point coordinates
    for point in paddle_points:
        x_coords = [frame['paddle'][point]['x'] for frame in data]
        y_coords = [frame['paddle'][point]['y'] for frame in data]
        paddle_coords.append((x_coords, y_coords))

    # Vectorized interpolation
    interpolated_paddle = vectorized_interpolate(paddle_coords)

    # Batch smoothing
    for idx, point in enumerate(paddle_points):
        x_array, y_array = interpolated_paddle[idx]

        # Check if enough valid points for smoothing
        original_x = [frame['paddle'][point]['x'] for frame in data]
        valid_count = sum(1 for x in original_x if x is not None)

        if valid_count > window_length:
            smooth_x = savgol_filter(x_array, window_length, polyorder)
            smooth_y = savgol_filter(y_array, window_length, polyorder)

            # Update only originally valid points
            for i in range(len(data)):
                if original_x[i] is not None:
                    data[i]['paddle'][point].update({
                        'x': float(smooth_x[i]),
                        'y': float(smooth_y[i])
                    })

    output_file = input_file.replace(').json', '_smoothed).json')
    with open(output_file, 'w') as f:
        json.dump(data, f, indent=2)

    return output_file

if __name__ == "__main__":
    start_time = time.time()
    input_path = "1209/1209_side(2D_trajectory).json"
    smoothed_data = smooth_2D_trajectory(input_path)
    print(f"Execution time: {time.time() - start_time:.4f}s")