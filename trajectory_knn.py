...
def analyze_trajectory(knn_dataset, input_3d_json, n_neighbors=1):
    ...
    results = [labels[idx] for idx in indices[0]]

    # ✅ 產生要寫入 txt 的文字（取第一名或整合前幾名）
    best_text = str(results[0]) if results else "None"

    # ✅ 產生 txt 檔名：支援 (3D_trajectory_smoothed).json 以及 _only_swing.json
    txt_path = (
        input_3d_json
        .replace('(3D_trajectory_smoothed).json', '_knn_feedback.txt')
        .replace('_only_swing.json', '_knn_feedback.txt')
    )

    # ✅ 寫出 txt（讓 GPT 模組用路徑讀它）
...
def analyze_trajectory(knn_dataset, input_3d_json, n_neighbors=1):
    ...
    for entry in dataset:
        ...
        for frame in frames:
            ...
            ball = frame['tennis_ball']
            paddle = frame.get('paddle_center', {"x": None, "y": None, "z": None})
