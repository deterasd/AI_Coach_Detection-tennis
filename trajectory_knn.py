...
def analyze_trajectory(knn_dataset, input_3d_json, n_neighbors=1):
    ...
    for entry in dataset:
        ...
        # 優先使用 suggestion，如果為空則使用 label，如果都沒有則使用 level 或檔名
        suggestion = entry.get('suggestion', '')
        label = entry.get('label', '')
        level = entry.get('level', '')
        filename = entry.get('filename', 'unknown')
        
        # 如果 suggestion 和 label 都是空字串，使用 level 或檔名作為標籤
        if suggestion and suggestion.strip():
            label_value = suggestion
        elif label and label.strip():
            label_value = label
        elif level:
            label_value = f"{level}_level"
        else:
            # 使用檔名作為最後的 fallback
            label_value = filename.replace('(3D_trajectory_smoothed).json', '').replace('(3D_trajectory).json', '')
        ...
        for frame in frames:
            ...
            paddle = frame.get('paddle_center') or (frame.get('paddle') or {}).get('center') or {}
...
def normalize_data(dataset, trajectory_filename):
    ...
    for data in dataset:
        for frame in data["data"]:
            for key in keys:
                if key in frame and key in normalized_data and frame_idx < len(normalized_data[key]):
...
def find_nearest_suggestion(trajectory_frames, merged_dataset, trajectory_filename):
    ...
    return filename_distances[best_filename][1], best_filename
...
def analyze_trajectory(merged_dataset_path, dynamic_filename):
    ...
    best_suggestion, nearest_expert_filename = find_nearest_suggestion(normalized_trajectory, merged_dataset, dynamic_filename)
    ...
    # Print result
    return suggestion_path, nearest_expert_filename

if __name__ == "__main__":
    merged_dataset_path = "knn_dataset_new.json"
