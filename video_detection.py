...
def process_video(
    ...
    for i in range(total_frames):
        ...
        # --- 畫球拍：四點 + 外框 + 對角十字 (X) ---
 #       if paddle_pts and len(paddle_pts) >= 7:
 ...
 #           cv2.line(frame, pts_sorted[1], pts_sorted[3], (0, 255, 255), 2)
        ...
        if paddle_pts and len(paddle_pts) > 0:
            for idx, (px, py) in enumerate(paddle_pts):
                ...
                # 設定顏色：Center(4)紅, Grip(5,6)綠, Rim(0-3)藍
                if idx == 4:   color = (0, 0, 255)
                elif idx >= 5: color = (0, 255, 0)
                else:          color = (255, 0, 0)
        ...
        if paddle_pts is not None:
            for j, label in enumerate(paddle_labels):
                if j < len(paddle_pts):
                    ...
                    cv2.putText(info_panel, f"{label:<8}: {pt}  Conf={conf:.2f}",
